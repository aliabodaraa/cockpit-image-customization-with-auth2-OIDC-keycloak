        ...
        if ($auth = $this->param('auth')) {
            if($this->app->request->is('ajax')){
                ...
                //}
                 ...
                 if (!isset($auth['user']) || !\is_string($auth['user'])) {
                    return ['success' => false, 'error' => 'Pre-condition failed'];
                }
//var_dump($auth);
		//$auth = ['user' => $auth['user']];
		//$mapper_type = $this->param('mapper_type');
                if (isset($auth['user']) && $this->app->helper('utils')->isEmail($auth['user'])) {
                    $auth['email'] = $auth['user'];
                    $auth['user']  = '';
                }
                ...
                $user = $this->module('cockpit')->authenticateViaOIDC($auth);
		...
		//var_dump($this->module('cockpit')->getUser());//here

		$b=new \Cockpit\AuthController($this->app);
