...
//$_SESSION['access_token']var_dump("SFSDFSDF");
});

// check + validate session time
$app->bind('/check-backend-session', function($app) {
    ...
    if($_SESSION['access_token']){ //auth session test --eklse exist before-- i add if
	 //introspect token for logout
         include_once($_SERVER['DOCUMENT_ROOT'] . '/OIDC_Keycloak_Cockpit/operations/introspectToken.php');
         $status = introspectToken();
	 //$_SESSION['cockpit.session.time']=0;
//	 $this->module('cockpit')->logout();
    }elseif($status && isset($_SESSION['cockpit.session.time']) && ($_SESSION['cockpit.session.time'] + $this->retrieve('session.lifetime', 10) < time())) { // check for inactivity: 45min by default
         $this->module('cockpit')->logout();
         $status = false;
     }
